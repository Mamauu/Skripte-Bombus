...
f.write("index zval_ne target Potential(Dipol) Dipol_dev(eA) steered_distance (A)\n")
f.close()
...
for i in range(1,steps+1):
	...
	f.write(str(i)+" "+str("{0:0.3f}".format(zval))+" "+str("{0:0.3f}".format(target))+" "+str("{0:0.5f}".format(potential_dipol))+" "+str("{0:0.5f}".format(dipole_dev[0]))+"\n")
	f.close()
